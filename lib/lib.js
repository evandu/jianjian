...
Lib.generatePayParams = function (wechatConfig,PrePayId) {
    ...
    logger.debug("pay generatePayParams", payParams)
    return payParams
}


Lib.readData = function *(path) {
...
Lib.sendOrderToWechat = function*(order1, wechatConfig, clientIp) {
    ...
    logger.debug("send order to wechat req data:",req)
    const response = yield request(req);
    logger.debug("send order to wechat response body:%s statusCode:%s" , response.body,response.statusCode)
