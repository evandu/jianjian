...
Lib.sendOrderToWechat = function*(order1, wechatConfig, clientIp) {
    ...
    const originData = `appid=${wechatConfig.appid}&body=${OrderData.body}&device_info=${OrderData.device_info}&mch_id=${wechatConfig.mch_id}&nonce_str=${OrderData.nonce_str}&key=${wechatConfig.key}`;
    ...
    if (response.statusCode == 200 && response.body && response.body !='' ) {
        const respData = (xml2json.toJson(response.body, {trim: true})).xml;
